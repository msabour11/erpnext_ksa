...
def create_qr_code(doc, method=None):
    ...
    if "ksa_einv_qr" in [d.fieldname for d in meta.get_image_fields()]:
        ...
        seller_name = frappe.db.get_value(
            "Company", doc.company, "company_name_in_arabic"
        )
    ...
    for row in doc.items:
        ...
        if itemised_tax.get(item_code):
            for tax in itemised_tax.get(item_code).values():
                _tax_rate = flt(
                    tax.get("tax_rate", 0),
                    row.precision("tax_rate") if row.get("tax_rate") else 2,
                )
                tax_amount += flt(
                    (row.net_amount * _tax_rate) / 100,
                    row.precision("tax_amount") if row.get("tax_amount") else 2,
                )
                tax_rate += _tax_rate

        row.tax_rate = flt(
            tax_rate, row.precision("tax_rate") if row.get("tax_rate") else 2
        )
        row.tax_amount = flt(
            tax_amount, row.precision("tax_amount") if row.get("tax_amount") else 2
        )
        row.total_amount = flt(
            (row.net_amount + row.tax_amount),
            row.precision("total_amount") if row.get("total_amount") else 2,
        )
